...
rf_avg_p_sd<-sd(data022822_p$rf_avg, na.rm = TRUE)
rf_avg_t_mean<-colMeans(data022822[data022822$Role=="T",][,'rf_avg']) # this value should be 0.571
rf_avg_T_sd<-sd(data022822_t$rf_avg, na.rm = TRUE)
...
# Fig 1A: Mean pepayment frequency distribution between therapists and patients
...
# Fig 1B: Correlation between mean repayment frequency and therapeutic alliance
...
# Combine Fig 1A, 1B into Fig 1.

Fig1 <- ggarrange(Fig1A, Fig1B, ncol=2, nrow=1, common.legend = TRUE)
ggsave("Fig1.png", width=8, height=3)
...
mixed_filtered <- mixed %>% 
  filter(!is.na(r) & !is.na(d_rf)) #75 rounds should be lost due to NA data. 
...
ggsave("Fig2.png", width=8, height=4) 
...
t.test(d_rf~intention, paired=FALSE, vars.equal=TRUE,
       data=mixed_t_without_neutral) # Note: not exactly matched or paired, i.e. some patients had more malevolent rounds than benevolent rounds, error in complete.cases(x,y): not all arguments have the same length. only patient data show sig differences between conditions
...
test_mixed_p<-mixed_p %>% 
  distinct(ID, .keep_all = TRUE) # trim down the dataframe to isolate clinical variable that is unique to each partcipant
...
# Run two extra 2 x 2 ANOVA to test impact of attachment anxiety (model of self), and by attachment avoidance (model of others) on changes in repayment fraction.
...
# Now, summarize the mean changes in repayment fraction across social reciprocity and alliance conditions 
...
# Run correlation test between mean fractional change in repayment and individual alliance ratings within each social reciprocity condition (malevolent vs. benevolent)
...
cor.test(mean_d_rf, Anxiety, alternative="two.sided", method="pearson") # how about anxiety group since its F score was high in 2 x 2 ANOVA? not sig in benevolent conditions, also not sig in malevolent conditions; but trend is anxiety increases decrease in repayment in malevolent conditions.

attach(d_rf_by_anxiety_p)
plot(mean_d_rf, Anxiety) 
...
ggsave("Fig3.png", height=4, width=8)
